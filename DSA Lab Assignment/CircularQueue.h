...
public:
	...
	bool operator==( CircularQueue<T> obj)
	{
		if (obj.maxSize()!= this->maxSize() || obj.size() != this->size())
		{
			return false;
		}
		else 
		{
			...
			if (QueueADT<T>::arr[QueueADT<T>::rear] != obj[i])
			{
				return false;
			}
		}
